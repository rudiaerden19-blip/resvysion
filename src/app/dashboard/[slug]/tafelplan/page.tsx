...
interface TableReservation {
  ...
  confirmed_by_customer: boolean; whatsapp_sent: boolean; created_at?: string
}
  ...
  return (
    <div style={{ width: totalW, height: totalH, position: 'relative' }}>
      ...
      <div style={{
        ...
        backgroundColor: tableColor, borderRadius: shape === 'round' ? '50%' : 10,
        ...
        <span style={{ color: '#fff', fontWeight: 700, fontSize: 13 }}>#{table.table_number}</span>
        <span style={{ color: 'rgba(255,255,255,0.75)', fontSize: 10, marginTop: 2 }}>{seats} pl.</span>
      </div>
      {reservationCount > 0 && (
        <div style={{
          position: 'absolute', top: padH - 10, right: padV - 10, width: 22, height: 22, borderRadius: '50%',
          backgroundColor: isOccupied ? '#DC2626' : '#1D4ED8', color: '#fff', fontSize: 11, fontWeight: 800,
          display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #fff', zIndex: 20,
        }}>
          {reservationCount}
        </div>
      )}
...
export default function TafelplanPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    ...
    loadUnassigned()
  }, [slug, agendaDate, tables.length, refreshToken])
  ...
  // Polling elke 30s
  useEffect(() => {
    const interval = setInterval(() => { loadBadges(agendaDate); if (selectedTable) loadAgenda(selectedTable.id, agendaDate) }, 30000)
    return () => clearInterval(interval)
  }, [slug, agendaDate, selectedTable?.id])

  // Real-time updates
  useEffect(() => {
    const channel = supabase.channel(`reservations-rt-${slug}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'reservations', filter: `tenant_slug=eq.${slug}` }, () => {
        loadBadges(agendaDate)
        if (selectedTable) loadAgenda(selectedTable.id, agendaDate)
      }).subscribe()
  ...
  async function loadData() {
    const [{ data: sec }, { data: tbl }] = await Promise.all([
      supabase.from('sections').select('*').eq('tenant_slug', slug).order('sort_order'),
      supabase.from('restaurant_tables').select('*').eq('tenant_slug', slug).eq('is_active', true),
    ])
    setSections(sec || [])
    setTables(tbl || [])
    setLoading(false)
  ...
  async function loadBadges(date: string) {
    const { data } = await supabase.from('reservations').select('*').eq('tenant_slug', slug).eq('reservation_date', date).eq('status', 'confirmed')
    const map: Record<string, TableReservation[]> = {}
    for (const r of data || []) { if (r.table_id) { if (!map[r.table_id]) map[r.table_id] = []; map[r.table_id].push(r) } }
    setBadgeMap(map)
  }

  async function loadAgenda(tableId: string, date: string) {
    const { data } = await supabase.from('reservations').select('*').eq('table_id', tableId).eq('reservation_date', date).in('status', ['confirmed', 'completed']).order('time_from', { ascending: true })
    setAgendaReservations(data || [])
  }

  async function loadUnassigned() {
    const today = new Date().toISOString().split('T')[0]
    const { data } = await supabase.from('reservations').select('*').eq('tenant_slug', slug).eq('status', 'confirmed').is('table_id', null).gte('reservation_date', today).order('reservation_date').order('time_from')
    setUnassigned(data || [])
  }
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    const newX = Math.max(0, dragging.current.ox + dx); const newY = Math.max(0, dragging.current.oy + dy)
  ...
  const onTouchEnd = useCallback((_e: React.TouchEvent, table: RestaurantTable) => {
    if (!isDragging.current) setSelectedTable(table)
    isDragging.current = false; dragging.current = null
  }, [])
  ...
  async function addSection() {
    ...
    const { data } = await supabase.from('sections').insert({ tenant_slug: slug, name: sectionForm.name.trim(), color: sectionForm.color, sort_order: sections.length }).select().single()
  ...
  async function addTable() {
    ...
    const { data } = await supabase.from('restaurant_tables').insert({ tenant_slug: slug, section_id: tableForm.section_id || null, table_number: tableForm.table_number.trim(), seats: tableForm.seats, shape: tableForm.shape, pos_x: px, pos_y: py, is_active: true }).select().single()
  ...
  async function deleteTable() {
    ...
    setTables(p => p.filter(t => t.id !== selectedTable.id)); setSelectedTable(null); setShowDeleteTable(false)
  }
  ...
  async function saveReservation() {
    ...
    const payload = {
      tenant_slug: slug, table_id: selectedTable.id, customer_name: fullName,
      customer_phone: resForm.customer_phone.trim(), customer_email: resForm.customer_email.trim(),
      party_size: resForm.party_size, reservation_date: resForm.date, reservation_time: resForm.time_from,
      time_from: resForm.time_from, time_to: resForm.time_to, notes: resForm.notes.trim() || null,
      deposit_amount: resForm.deposit_amount, deposit_paid: resForm.deposit_paid, status: 'confirmed', is_occupied: false,
    }
    if (editingRes) {
      await supabase.from('reservations').update(payload).eq('id', editingRes.id)
    } else {
      const { data: newRes } = await supabase.from('reservations').insert(payload).select('id').single()
      if (newRes && resForm.customer_phone.trim()) {
        fetch('/api/whatsapp/send-reservation-confirmation', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reservationId: newRes.id, tenantSlug: slug }) }).catch(() => {})
      }
    }
  ...
  async function markOccupied(res: TableReservation) {
    await supabase.from('reservations').update({ is_occupied: true }).eq('id', res.id); await refreshAll()
  }

  async function releaseTable(res: TableReservation) {
    await supabase.from('reservations').update({ is_occupied: false, status: 'completed' }).eq('id', res.id); await refreshAll()
  }

  async function cancelReservation(res: TableReservation) {
    await supabase.from('reservations').update({ status: 'cancelled' }).eq('id', res.id); await refreshAll()
  }
  ...
  async function loadArchive(filter: 'dag'|'week'|'maand'|'jaar') {
    ...
    const { data } = await supabase.from('reservations').select('*').eq('tenant_slug', slug).in('status', ['completed','cancelled']).gte('reservation_date', from.toISOString().split('T')[0]).lte('reservation_date', now.toISOString().split('T')[0]).order('reservation_date', { ascending: false })
    const enriched = (data || []).map(r => ({ ...r, table_number: tables.find(t => t.id === r.table_id)?.table_number || '?' }))
    setArchiveData(enriched)
  }

  async function assignTable(res: TableReservation, tableId: string) {
    await supabase.from('reservations').update({ table_id: tableId, whatsapp_sent: false }).eq('id', res.id)
    fetch('/api/whatsapp/send-reservation-confirmation', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reservationId: res.id, tenantSlug: slug }) }).catch(() => {})
    setAssigningRes(null); setAgendaDate(res.reservation_date); await refreshAll()
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Tafelplan</h1>
          <p className="text-gray-500">{tables.length} tafels</p>
        </div>
        <div className="flex gap-2 items-center flex-wrap">
          ...
          <button onClick={() => setShowAddSection(true)} className="px-3 py-2 border-2 border-gray-200 text-gray-700 rounded-xl font-medium hover:bg-gray-50 text-sm">+ Zone</button>
          <button onClick={() => { setTableForm({ table_number: `${tables.length + 1}`, seats: 4, shape: 'square', section_id: '' }); setShowAddTable(true) }} className="px-3 py-2 border-2 border-blue-200 text-blue-700 rounded-xl font-medium hover:bg-blue-50 text-sm">+ Tafel</button>
          <button onClick={() => setShowOnlinePanel(true)} className="relative px-3 py-2 border-2 border-orange-200 text-orange-700 rounded-xl font-medium hover:bg-orange-50 text-sm">
            📬 Online
            {unassigned.length > 0 && <span className="absolute -top-2 -right-2 w-5 h-5 bg-red-600 text-white text-xs font-black rounded-full flex items-center justify-center">{unassigned.length}</span>}
          </button>
          <button onClick={savePositions} disabled={saving} className={`px-4 py-2 rounded-xl font-bold text-sm transition-colors ${savedMsg ? 'bg-green-600 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'} disabled:opacity-60`}>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
          ...
          {(['dag','week','maand','jaar'] as const).map(f => (
            <button key={f} onClick={() => { setArchiveFilter(f); setSelectedTable(null); loadArchive(f) }}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold capitalize transition-colors ${archiveFilter === f ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
              {f.charAt(0).toUpperCase() + f.slice(1)}
            </button>
          ))}
          {archiveFilter !== 'none' && <button onClick={() => setArchiveFilter('none')} className="px-2 py-1.5 text-xs text-gray-400 hover:text-gray-600">✕</button>}
        </div>
      </div>
      ...
      {archiveFilter !== 'none' && (
        <div className="bg-white rounded-2xl border-2 border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b bg-gray-50 flex items-center justify-between">
            <div><h2 className="font-bold text-gray-900">Archief — {archiveFilter}</h2><p className="text-sm text-gray-500">{archiveData.length} reservaties</p></div>
            <button onClick={() => setArchiveFilter('none')} className="text-gray-400 hover:text-gray-600 text-xl font-bold">×</button>
          </div>
          {archiveData.length === 0 ? (
            <div className="py-12 text-center text-gray-400"><span className="text-4xl block mb-2">📭</span><p>Geen reservaties voor deze periode</p></div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b">
                  <tr>{['Datum','Uur','Tafel','Naam','Tel','Pers.','Status'].map(h => <th key={h} className="text-left px-4 py-3 font-semibold text-gray-600">{h}</th>)}</tr>
                </thead>
                <tbody>
                  {archiveData.map((r, i) => (
                    <tr key={r.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      <td className="px-4 py-3">{new Date(r.reservation_date + 'T12:00').toLocaleDateString('nl-BE', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                      <td className="px-4 py-3">{r.time_from || r.reservation_time}{r.time_to ? ` → ${r.time_to}` : ''}</td>
                      <td className="px-4 py-3 font-bold">#{r.table_number || '?'}</td>
                      ...
                      <td className="px-4 py-3">
                        {r.status === 'completed' ? <span className="bg-gray-200 text-gray-600 text-xs font-bold px-2 py-0.5 rounded-full">VOLTOOID</span> : <span className="bg-red-100 text-red-600 text-xs font-bold px-2 py-0.5 rounded-full">GEANNULEERD</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="relative rounded-2xl overflow-hidden select-none"
            ...
            onTouchMove={onTouchMove} onTouchEnd={() => { dragging.current = null }}
            ...
            {tables.map(table => {
              ...
              return (
                <div key={table.id} onMouseDown={e => onMouseDown(e, table)} onClick={e => onTableClick(e, table)}
                  onTouchStart={e => onTouchStart(e, table)} onTouchEnd={e => onTouchEnd(e, table)}
                  className="absolute cursor-pointer touch-none" style={{ left: table.pos_x, top: table.pos_y, zIndex: selectedTable?.id === table.id ? 10 : 1 }}>
                  <TableWithChairs table={table} color={getTableColor(table)} isSelected={selectedTable?.id === table.id} reservationCount={tableResv.length} isOccupied={tableResv.some(r => r.is_occupied)} />
                </div>
              )
            })}
            <div className="absolute top-3 left-3 bg-black/40 backdrop-blur-sm px-3 py-1.5 rounded-lg text-xs text-white/90">Klik voor agenda · Sleep om te verplaatsen</div>
          </div>
        </div>
                ...
                return (
                  <div key={res.id} className={`rounded-2xl border-2 p-4 ${occupied ? 'border-red-200 bg-red-50' : 'border-green-200 bg-green-50'}`}>
                    <div className="flex items-center justify-between mb-2">
                      <span className={`text-base font-black ${occupied ? 'text-red-700' : 'text-green-700'}`}>{res.time_from || res.reservation_time} → {res.time_to || '?'}</span>
                      <div>
                        {res.confirmed_by_customer ? <span className="bg-green-100 text-green-700 text-xs font-bold px-2 py-0.5 rounded-full">✅ Bevestigd</span> : res.whatsapp_sent ? <span className="bg-blue-50 text-blue-500 text-xs font-medium px-2 py-0.5 rounded-full">💬 WA verzonden</span> : <span className="text-xs text-gray-400">{res.party_size} pers.</span>}
                      </div>
                    </div>
                    ...
                    <div className="text-sm text-gray-600 space-y-0.5 mt-1">
                      ...
                      {res.notes && <p className="text-xs italic text-gray-400">&quot;{res.notes}&quot;</p>}
                    </div>
                    <div className="flex gap-2 mt-3">
                      {!occupied ? (
                        <button onClick={() => markOccupied(res)} className="flex-1 py-2 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-700">✅ Gereserveerd</button>
                      ) : (
                        <button onClick={() => releaseTable(res)} className="flex-1 py-2 bg-red-600 text-white rounded-lg text-xs font-bold hover:bg-red-700">🔴 Tafel vrijgeven</button>
                      )}
                      <button onClick={() => { const parts = (res.customer_name || '').split(' '); setResForm({ customer_first_name: parts[0] || '', customer_last_name: parts.slice(1).join(' ') || '', customer_phone: res.customer_phone, customer_email: res.customer_email, party_size: res.party_size, date: res.reservation_date, time_from: res.time_from || res.reservation_time, time_to: res.time_to || '', notes: res.notes || '', deposit_amount: res.deposit_amount, deposit_paid: res.deposit_paid }); setEditingRes(res); setShowResForm(true) }} className="px-3 py-1.5 bg-white border border-gray-200 text-gray-600 rounded-lg text-xs hover:bg-gray-50">✏️</button>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowAddSection(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3"><button onClick={() => setShowAddSection(false)} className="flex-1 py-3 border-2 border-gray-200 rounded-xl font-medium">Annuleren</button><button onClick={addSection} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold">Toevoegen</button></div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowAddTable(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3"><button onClick={() => setShowAddTable(false)} className="flex-1 py-3 border-2 border-gray-200 rounded-xl font-medium">Annuleren</button><button onClick={addTable} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold">Toevoegen</button></div>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Voornaam <span className="text-red-500">*</span></label><input autoFocus value={resForm.customer_first_name} onChange={e => { setResForm(p => ({ ...p, customer_first_name: e.target.value })); setResErrors(p => ({ ...p, first_name: false })) }} placeholder="Voornaam" className={`w-full border-2 rounded-xl px-3 py-2.5 focus:outline-none ${resErrors.first_name ? 'border-red-400 bg-red-50' : 'border-gray-200 focus:border-blue-500'}`} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Achternaam <span className="text-red-500">*</span></label><input value={resForm.customer_last_name} onChange={e => { setResForm(p => ({ ...p, customer_last_name: e.target.value })); setResErrors(p => ({ ...p, last_name: false })) }} placeholder="Achternaam" className={`w-full border-2 rounded-xl px-3 py-2.5 focus:outline-none ${resErrors.last_name ? 'border-red-400 bg-red-50' : 'border-gray-200 focus:border-blue-500'}`} /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Telefoon <span className="text-red-500">*</span></label><input value={resForm.customer_phone} onChange={e => { setResForm(p => ({ ...p, customer_phone: e.target.value })); setResErrors(p => ({ ...p, phone: false })) }} placeholder="+32 4xx..." className={`w-full border-2 rounded-xl px-3 py-2.5 focus:outline-none ${resErrors.phone ? 'border-red-400 bg-red-50' : 'border-gray-200 focus:border-blue-500'}`} /></div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4" onClick={() => setShowOnlinePanel(false)}>
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div><h3 className="text-xl font-bold text-gray-900">📬 Online reservaties</h3><p className="text-sm text-gray-500">{unassigned.length === 0 ? 'Alle reservaties zijn toegewezen' : `${unassigned.length} wacht op tafeltoewijzing`}</p></div>
              <button onClick={() => setShowOnlinePanel(false)} className="text-gray-400 hover:text-gray-600 text-2xl font-bold">×</button>
            </div>
            <div className="overflow-y-auto flex-1 p-4 space-y-3">
              {unassigned.length === 0 ? (
                <div className="text-center py-12 text-gray-400"><span className="text-5xl block mb-3">✅</span><p>Alle online reservaties zijn toegewezen</p></div>
              ) : unassigned.map(res => (
                <div key={res.id} className="border-2 border-orange-100 bg-orange-50 rounded-xl p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <p className="font-bold text-gray-900">{res.customer_name}</p>
                      <p className="text-sm font-semibold text-orange-600 mt-0.5">📅 {new Date(res.reservation_date + 'T12:00').toLocaleDateString('nl-BE', { weekday: 'long', day: 'numeric', month: 'long' })} · {res.time_from}{res.time_to ? ` → ${res.time_to}` : ''}</p>
                    ...
                    <button onClick={() => { setAssigningRes(res); setShowOnlinePanel(false) }} className="flex-shrink-0 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-sm font-bold rounded-xl">🪑 Tafel toewijzen</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setAssigningRes(null)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {tables.map(t => (
                <button key={t.id} onClick={() => assignTable(assigningRes, t.id)} className="w-full flex items-center justify-between px-4 py-3 border-2 border-gray-200 rounded-xl hover:border-blue-400 hover:bg-blue-50 text-left">
